...
const Item: React.FC<Props> = ({device, deleteDevice, editDevice}) => {
    ...
    function handleManageLight() {
        ...
        setLedOn(device.state)
        const color = device.state ? '446AD9' : '000000';
        manageLed(id, color, device.state)
            .then(({status}) => {
                if (status !== 200) {
                    throw new Error("Error! Light can't be managed")
                }
                console.log("changed", device.state)
            })
    ...
    return (
        <div className="grid-container">
            ...
            <Card className="card-admin"
                  style={{borderRadius: '20px', overflow: 'hidden', backgroundColor: 'rgba(255,255,255,0.8)'}}>
                ...
                <div className="people">
                    ...
                    {device.type === type.CAMERA && (
                        <div>
                            <div className="switch">
                                <label>
                                    Off
                                    <input type="checkbox" checked={cameraOn} onChange={handleManageCamera}/>
                    ...
                    {device.type === type.LIGHT && (
                        <div>
                            <button className="material-icons change-color-button purple darken-1"
                                    onClick={handleColorPickerOpen}>color_lens
                            </button>
                            {isColorPickerActive && (
                    ...
                    {device.type === type.LAMP && (
                        <div>
                            <button className="material-icons alarm-button purple darken-1"
                                    onClick={handleManageClockAlarmPickerOpen}>access_alarm
                            </button>
                            ...
                            <button className="material-icons alarm-button purple darken-1"
                                    onClick={handleManageClockTimePickerOpen}>access_time
                            </button>
                            ...
                            <div className="switch">
                                <label>
                                    Off
                                    <input type="checkbox" checked={lampOn} onChange={handleManageLamp}/>
                                    <span className="lever"></span>
                                    On
                                </label>
                            </div>
                    ...
                    <button className="material-icons delete-button pink darken-3"
                            onClick={() => deleteDevice(parseInt(device.id))}>delete
                    </button>
                    <button className="material-icons delete-button purple darken-1"
